...
function validateMovieInput(body) {
  ...
  if (body.year && (Number(body.year) < 1888 || Number(body.year) > 2100)) errors.push('Year must be between 1888 and 2100.');
  if (!body.rating) errors.push('Rating is required.');
  if (body.rating && (Number(body.rating) < 0 || Number(body.rating) > 10)) errors.push('Rating must be between 0 and 10.');
  if (normalizeGenres(body.genres).length === 0) errors.push('Enter at least one genre.');
